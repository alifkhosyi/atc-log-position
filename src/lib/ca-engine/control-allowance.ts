 ...
 *
 * NOTE (split-engine refactor): file pindah dari roster-engine/control-allowance.ts.
 * Logic UNCHANGED — pure move + import path update.
 */

import type {
    PersonnelAllowance, AllowanceSummary, AllowanceResult,
    ComputeAllowanceOptions,
} from './types';
import { SHIFT_TOKENS } from '../shared/types';
import type { GenerateResult } from '../roster-engine/types';
import type { RollingConfig, UnitConfig } from '../airport-data/types';
import {
    POSITION_KONTROL, POSITION_ASISTEN,
} from '../airport-data/types';
import { getCAConstant } from '../airport-data/loader';
import {
...
export function computeAllowanceTable(opts: ComputeAllowanceOptions): AllowanceResult {
    ...
    // Aggregate overtime per personnel_id (keyed by whatever caller passes —
    // typically initial for engine output match, or UUID as defensive fallback).
